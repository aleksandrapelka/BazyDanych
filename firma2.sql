-- 1. Tworzenie nowej bazy danych o nazwie firma_2:
...
UPDATE ksiegowosc.pensja SET kwota = ROUND(kwota - 1000.00, 2);
...
FROM ksiegowosc.pracownicy  INNER JOIN (ksiegowosc.godziny INNER JOIN (ksiegowosc.premia INNER JOIN ksiegowosc.wynagrodzenie ON premia.ID_premii = wynagrodzenie.ID_premii)  ON godziny.ID_godziny = wynagrodzenie.ID_godziny) ON pracownicy.ID_pracownika = wynagrodzenie.ID_pracownika 
...
SELECT AVG(pensja.kwota) AS Œrednia, MIN(pensja.kwota) AS Minimalna, MAX(pensja.kwota) AS Maksymalna
FROM ksiegowosc.pensja
WHERE pensja.stanowisko LIKE 'asystent' OR pensja.stanowisko LIKE 'administrator IT'

SELECT * FROM ksiegowosc.pensja

-- m) Policz sumê wszystkich wynagrodzeñ:
SELECT SUM(pensja.kwota) AS SumaPensji, SUM(premia.kwota)  AS SumaPremii, SUM(pensja.kwota) + SUM(premia.kwota) AS SumaWynagrodzeñ
...
SELECT pracownicy.ID_pracownika, pracownicy.imie, pracownicy.nazwisko, pensja.kwota
FROM ksiegowosc.pracownicy INNER JOIN ( ksiegowosc.pensja INNER JOIN ksiegowosc.wynagrodzenie ON pensja.ID_pensji = wynagrodzenie.ID_pensji) ON pracownicy.ID_pracownika = wynagrodzenie.ID_pracownika
WHERE pensja.kwota <= 2300

--DELETE ksiegowosc.pracownicy FROM ksiegowosc.premia INNER JOIN (ksiegowosc.pensja INNER JOIN ksiegowosc.wynagrodzenie ON pensja.ID_pensji = wynagrodzenie.ID_pensji) ON premia.ID_premii = wynagrodzenie.ID_premii 
--WHERE pensja.kwota <= 2300; 
...
DELETE pracownicy FROM ksiegowosc.pracownicy WHERE ID_pracownika = 3 or ID_pracownika = 6 or ID_pracownika = 9 